...
图像识别领域的核心突破建立在深度卷积神经网络（CNN）的发展上[24][25]。Simonyan 和 Zisserman（2014）提出的 VGGNet 通过堆叠小尺寸（3×3）卷积核验证了网络深度对特征质量的决定性影响，VGG-16 和 VGG-19 在 ImageNet 上的表现为后续深层网络设计提供了重要基准[1]。He 等人（2016）进一步提出了残差网络（ResNet），通过跳跃连接（Skip Connection）解决了深层网络训练中的梯度消失问题，将可训练的有效深度从十几层推进到数百层，ResNet-50 在此基础上成为图像特征提取的主流骨干网络，被广泛应用于下游迁移学习任务[2]。

Hu 等人（2018）提出的 Squeeze-and-Excitation 网络（SENet）引入通道注意力机制，使网络能够自适应地重新校准不同通道的特征响应权重，在细粒度识别任务中尤为有效，因为不同通道可能对鼻纹纹路的不同局部模式有不同的感知能力[3]。

这些工作共同构成了本研究特征提取模块的技术基础。ResNet-50 作为骨干网络的选择，正是基于其在迁移学习场景下的成熟表现和 2048 维特征空间提供的充足表达容量。

#### 1.2.2  度量学习与孪生网络

度量学习（Metric Learning）是解决小样本识别问题的核心技术路线[27]。Bromley 等人（1993）最早将孪生网络结构应用于手写签名验证，通过共享权重的双路网络输出特征向量，再用欧氏距离度量相似性，奠定了"比较而非分类"这一问题范式的基础[4]。Koch 等人（2015）将孪生网络推广至单样本图像识别任务，在 Omniglot 数据集上验证了该框架在极少样本条件下的有效性[5]。

在损失函数层面，对比损失（Contrastive Loss）由 Hadsell 等人（2006）提出，明确区分正样本对（同类）和负样本对（异类），通过同时拉近正样本对距离、推开负样本对距离来塑造特征空间[9]。Schroff 等人（2015）在 FaceNet 中引入三元组损失（Triplet Loss），以锚点-正样本-负样本三元组为单位设计梯度更新，使特征空间中同类个体聚集、异类个体分散的效果更显著，在人脸识别领域取得了当时最优的结果[6]。Snell 等人（2017）提出的 Prototypical Networks 通过在度量空间中计算类别原型距离完成分类，在小样本场景下实现了简洁有效的泛化，为本研究在有限样本条件下的方法设计提供了参考[7]。

Hermans 等人（2017）对多种三元组采样策略进行了系统性对比，发现在线批内难例挖掘（Batch Hard Mining）在大多数场景下优于随机负样本采样，这为本研究的样本对生成策略提供了参考[8]。

#### 1.2.3  动物个体识别与鼻纹研究

动物个体识别作为一个独立研究方向，近年来随着保育和宠物管理需求的增长而受到更多关注[21][23]。Schneider 等人（2019）对基于相机陷阱数据的动物个体重识别方法进行了系统综述，梳理了传统特征工程方法与深度学习方法的演进脉络，指出深度特征在复杂背景下的鲁棒性显著优于手工特征[21]。Vidal 等人（2021）进一步从生物学和计算机视觉的交叉视角，讨论了个体识别任务对特征稳定性的要求，犬鼻纹的乳突纹路结构在成年后终身稳定，完全满足这一前提[23]。

在犬类鼻纹识别的专项研究方向，方伟等人（2024）对犬鼻纹识别技术的研究现状进行了系统综述，梳理了图像采集条件对识别性能的影响，指出光照不均和拍摄角度偏差是主要干扰因素[10]。李梦晗等人（2024）在此基础上将 FaceNet 框架引入犬鼻纹识别任务，验证了度量学习方法在该细粒度生物特征识别场景的有效性[11]。方伟等人（2023）还对犬个体生物识别技术的多种模态进行了比较分析，指出鼻纹相对于耳标、芯片等标记方式的非侵入性优势[12]。

Choi 等人（2021）以比格犬为研究对象，追踪记录了从幼犬到成犬阶段鼻纹形态的演变规律，实验证明鼻纹纹路在出生后早期即趋于稳定，为鼻纹作为终身识别媒介提供了关键生物学证据[13]。Redmon 等人（2016）提出的 YOLO 系列目标检测框架，其快速区域提取能力在动物图像预处理（如鼻部区域精确裁剪）中有广泛应用价值，是动物识别系统前处理链路的常见选择[14]。
...
批归一化对每个 mini-batch 的特征进行归一化[15]，其数学形式为：
...
迁移学习（Transfer Learning）将在大规模数据集上预训练的模型权重迁移到目标任务，替代从随机初始化开始训练[17]。这一策略在小样本场景下尤为关键：预训练网络已在 ImageNet-1k（128 万张图片、1000 类）上学会了通用的视觉特征（边缘、纹理、形状）[18]，这些低级特征对绝大多数图像识别任务都有迁移价值。
...
孪生网络由 Bromley 等人（1993）提出，原始应用是手写签名验证[4]。其核心设计是一对共享权重的子网络，分别接受两路输入，各自提取特征向量，再通过距离度量判断两者相似程度。
...
对比损失（Contrastive Loss）由 Hadsell 等人（2006）提出[9]，定义为：
...
三元组损失（Triplet Loss）由 Schroff 等人（2015）在 FaceNet 中提出[6]，以锚点（anchor）、正样本（positive）、负样本（negative）三元组为单位，要求：

$$\mathcal{L}_{triplet} = \max(0, \|f(a) - f(p)\|^2 - \|f(a) - f(n)\|^2 + \alpha)$$
<div style="text-align:right">**(2.6)**</div>

其中 $\alpha$ 是 margin 参数，要求正样本比负样本至少近 $\alpha$。三元组损失在人脸识别领域验证了很强的效果[6]，但其训练需要精心设计的三元组采样策略，随机采样会产生大量对训练无贡献的"简单三元组"[8]。在本研究 batch_size=32 的配置下，有效难例三元组数量有限，对比损失的训练稳定性更好。

### 2.4  ResNet-50 骨干网络

#### 2.4.1  残差学习原理

ResNet 由何恺明等人（2016）提出，核心创新是残差学习框架[2]。在此之前，直接叠加更多层并不一定改善性能——随着网络深度增加，梯度消失问题使深层参数无法得到有效更新，网络实际上退化为一个浅层网络。
...
Hu 等人（2018）提出的 Squeeze-and-Excitation 网络（SENet）引入通道注意力（Channel Attention）机制[3]，核心思想是让网络自适应地学习各通道特征的重要性权重，动态强调有用通道、抑制无用通道。
...
在犬鼻纹识别场景，鼻纹纹路的细粒度差异往往集中在特定的纹理频率通道上，而与颜色相关的通道贡献较小。通道注意力让模型自主学习这种通道间的重要性差异，在细粒度识别任务上通常带来 0.5-1pp 的准确率提升[26]。ResNet-50 作为 SE-ResNet-50 的前身，其基础架构保持一致，仅在每个残差块后添加 SE 模块即可获得注意力增强版本。本研究采用标准 ResNet-50，保留了这一扩展可能性作为后续改进方向。
...
![图 3-1  系统整体三层架构](/Users/songjiayi4/Workspace/Dog-nose-recognition/network_training/experiments/results/figures/diagrams/fig_system_arch.png)

*图 3-1  系统整体三层架构*

图 3-1 展示了系统的三层架构及各层之间的调用关系。用户界面层（Frontend）基于 Next.js 15 构建，通过 HTTP/REST 协议与业务逻辑层通信；业务逻辑层（Backend）由 FastAPI 驱动，对外暴露 `/compare`、`/compare-files`、`/health` 三个核心端点，负责调用孪生网络完成推理；业务逻辑层向下分叉连接两个独立的存储节点——模型层（ML Model）持有 Siamese-ResNet50 权重文件，数据存储层（Supabase）同时承载 PostgreSQL 结构化档案和 Storage Bucket 非结构化鼻纹图片。
...
归一化参数使用 ImageNet 的统计值而非数据集本身的均值方差，原因是 ResNet-50 预训练权重是在这一归一化基准下训练的。若使用不同分布的归一化，会破坏预训练特征的激活模式，降低迁移效果。实践中，即使目标数据集（犬鼻纹图片）与 ImageNet 的像素分布有差异，沿用 ImageNet 参数的效果通常优于重新计算数据集均值方差，这一结论在迁移学习文献中有广泛验证[17][18]。
...
![图 3-2  孪生网络结构](/Users/songjiayi4/Workspace/Dog-nose-recognition/network_training/experiments/results/figures/diagrams/fig_siamese_arch.png)
...
模型的前向传播分三个阶段。首先，两张输入图片分别送入共享权重的 ResNet-50 骨干，各自提取得到 2048 维特征向量；其次，对两个特征向量逐元素作绝对值差，得到同样是 2048 维的差分向量；最后，差分向量依次经过三层全连接头（2048→256→128→1），输出一个标量 logit。

推理阶段对该 logit 施加 Sigmoid 激活，将其映射至 $(0, 1)$ 区间作为相似度概率：概率高于 0.5 判定为同一犬只，概率值乘以 100 即为百分比置信度，直接用于前端展示。
...
![图 3-3  训练流程](/Users/songjiayi4/Workspace/Dog-nose-recognition/network_training/experiments/results/figures/diagrams/fig_train_flow.png)

*图 3-3  训练流程*

图 3-3 展示了单个 Epoch 的完整训练循环。每轮 Epoch 开始时，数据加载器按批次提供样本对（img1、img2 及标签），孪生网络对每对图片执行前向传播并输出相似度得分，随后计算对比损失、执行反向传播、由 Adam 优化器完成参数更新，形成批内的"前向—损失—反向—更新"四步闭环。当一个 Epoch 的所有批次处理完毕后，进入验证阶段统计 val_loss 与 val_acc，并保存当前最优模型权重；若批次未用尽，则循环返回继续加载下一批次。每个 Epoch 结束后记录的训练损失、验证损失和验证准确率，用于后续训练曲线绘制和超参数分析。

#### 3.4.3  对比损失实现

对比损失的数学定义已在第 2.3.2 节给出，此处说明其在训练流程中的具体接入方式。损失函数接受两个输入：模型对一个样本对输出的相似度得分，以及该样本对的标签（0 表示同类正样本对，1 表示异类负样本对）。

实现上有一个关键细节：损失函数直接作用于模型的原始输出（logit），而非经过 Sigmoid 激活后的概率值。这样设计的原因是 Sigmoid 在极值区域梯度趋近于零，若先激活再计算损失，会在训练后期出现梯度消失；直接对 logit 施加对比损失可以保持梯度信号的有效传递。推理阶段再对 logit 应用 Sigmoid，将输出映射至 $[0, 1]$ 区间作为相似度概率，两者各司其职，不存在冲突。每批次的损失取所有样本对损失的均值后执行反向传播。

#### 3.4.4  Adam 优化器的选择

Adam（Adaptive Moment Estimation）由 Kingma 和 Ba（2015）提出[16]，为每个参数维护独立的自适应学习率，兼顾了梯度的一阶矩（动量）和二阶矩（梯度平方的移动平均）估计：
...
模型加载是后端启动的最慢步骤（ResNet-50 权重文件约 100MB，CPU 加载约需 3-5 秒）。后端采用懒加载单例模式：以模块级全局变量持有模型实例和设备信息，初始值均为空；每次推理请求到来时先检查全局变量是否已初始化，若为空则自动检测 CUDA 可用性确定运行设备，随后实例化 SiameseNetwork、加载权重文件并切换至评估模式，之后所有请求直接复用已加载的实例。

这一模式的优点是：FastAPI 服务启动时无需等待模型加载（启动时间从 5s+ 降至亚秒级），且只有在实际收到推理请求时才占用内存；缺点是第一次请求延迟略高，可通过在服务启动事件中主动触发一次空推理（warmup）来消除冷启动延迟。

#### 3.5.5  图像预处理（后端推理链路）

后端接收到图片 URL 后，依次执行三步转换：首先通过 HTTP 请求下载图片并解码为 RGB 格式；随后按与训练阶段完全一致的流水线处理——缩放至 224×224、转换为浮点张量、以 ImageNet 均值和标准差归一化；最后在批次维度扩展为四维张量后送入模型。预处理参数与训练时保持严格一致，确保推理时的输入分布不产生偏移。

#### 3.5.6  一键启动脚本

项目提供 `start.sh` 脚本，支持三种启动模式：不带参数时同时拉起前端（端口 3000）和后端（端口 8000）；传入 `backend` 参数时仅启动后端；传入 `frontend` 参数时仅启动前端。前端以开发模式运行并支持热重载，后端通过 Uvicorn 以 reload 模式启动，两者均可在代码修改后自动重新加载，方便开发阶段的快速迭代。
...
![学习率消融曲线](/Users/songjiayi4/Workspace/Dog-nose-recognition/network_training/experiments/results/figures/ablation_lr.png)
...
![批大小消融曲线](/Users/songjiayi4/Workspace/Dog-nose-recognition/network_training/experiments/results/figures/ablation_bs.png)
...
![Margin 消融曲线](/Users/songjiayi4/Workspace/Dog-nose-recognition/network_training/experiments/results/figures/ablation_margin.png)
...
![Backbone 消融曲线](/Users/songjiayi4/Workspace/Dog-nose-recognition/network_training/experiments/results/figures/ablation_backbone.png)
...
代价是参数量从 0.3M 增加到 25.9M，CPU 环境下单 Epoch 训练时间约增加 4 倍。在 GPU 环境下这一代价大幅减小，因此在有 GPU 资源的生产环境中，**强烈推荐开启 Backbone 微调**[17]。
...
![消融实验汇总](/Users/songjiayi4/Workspace/Dog-nose-recognition/network_training/experiments/results/figures/ablation_summary_bar.png)
...
**对照 A：VGG-16 闭集分类器**，以 Simonyan 等人（2014）提出的 VGG-16 为骨干（ImageNet 预训练）[1]，去除原 1000 类分类头，替换为 Linear(4096→1393) 输出层，使用交叉熵损失训练闭集 1393 类分类任务。VGG-16 是深度学习图像分类领域的经典基线，参数量 138.4M，计算量 15.5 GFLOPs，是三个模型中参数量最大的。

**核心局限**：闭集分类器的 softmax 输出依赖固定的训练类别空间。当系统需要注册新犬只时，必须将新样本加入训练集并重新训练整个网络，无法在推理阶段处理训练集以外的犬只（开放集识别失效）。

**对照 B：ResNet-50 闭集分类器**，以 He 等人（2016）提出的 ResNet-50 为骨干（ImageNet 预训练，与本研究孪生网络共享相同骨干）[2]，去除原分类头，替换为 Linear(2048→1393) 输出层，使用交叉熵损失训练。ResNet-50 分类器与本研究孪生网络的骨干结构完全一致，因此本对照能够**精确隔离学习范式的差异**，排除骨干差异的干扰。
...
![模型训练曲线对比](/Users/songjiayi4/Workspace/Dog-nose-recognition/network_training/experiments/results/figures/model_comparison_curves.png)

训练曲线揭示了三种模型在收敛行为上的本质差异。

**Siamese-ResNet50（蓝线）** 在测试损失和准确率两个维度均表现最优。度量学习的训练信号来自图像对的相似度标注，每张图片可参与多个样本对构成训练样本，有效训练信号密度远高于单张图片的分类标签[27]。ImageNet 预训练提供的纹理检测先验使模型在第 5 个 Epoch 附近已达到接近收敛状态，最终闭集准确率 87.20%，且开放集准确率同样为 88.10%，说明度量空间真正学到了跨类别的鼻纹相似性特征。
...
![闭集 vs 开放集准确率](/Users/songjiayi4/Workspace/Dog-nose-recognition/network_training/experiments/results/figures/model_params_vs_acc.png)
...
![综合性能雷达图](/Users/songjiayi4/Workspace/Dog-nose-recognition/network_training/experiments/results/figures/model_radar.png)
...
**小样本利用效率**，本数据集每类仅约 6 张图片（1393 类，8364 张），闭集分类器需要每类样本足够多以学习类内方差，极易过拟合；孪生网络通过构造图像对，将每张图片复用为多个训练样本对，有效数据量呈平方级放大，在小样本场景下具有天然优势[5][7]。

**特征空间的泛化性**，对比损失驱动模型学习类别无关的全局相似性结构[9]，嵌入空间的几何关系对新类别同样有效；交叉熵损失仅约束决策边界，未对特征空间的全局结构施加约束，对新类别无泛化能力。
...
从同骨干对比（Siamese-ResNet50 vs ResNet-50 分类器）来看，两者使用完全相同的 ResNet-50 骨干和 ImageNet 预训练权重，差距完全来自学习范式：对比损失的度量学习目标驱动特征空间形成可泛化的全局相似性结构，这一优势在开放集场景下最为显著，与 Moskvyak 等人（2021）在基于关键点对齐嵌入的图像检索与个体重识别任务中报告的度量学习泛化优势相一致[22]。
...
**样本对策略的局限**，当前在线随机负采样对难例负样本（与正样本视觉相近的不同个体）缺乏针对性。Hermans 等人（2017）的研究表明批内难例挖掘（Batch Hard Mining）在度量学习任务中通常优于随机采样[8]，引入这一策略有望进一步提升特征空间的判别性。
...
**数据增强**，当前预处理流水线未加入任何增强操作，而实际拍摄中鼻纹图像的光照条件、拍摄角度变化是准确率的主要限制因素之一。Shorten 和 Khoshgoftaar（2019）的综述表明，合理的数据增强策略在深度学习图像分类任务中通常带来显著的泛化提升[20]。建议加入的增强策略包括：随机水平翻转（概率 0.5，鼻纹具有左右对称性）、随机颜色抖动（亮度和对比度 ±20%，模拟光照变化）、随机仿射变换（旋转 ±15°，模拟拍摄角度偏差）、高斯噪声（σ=0.02，模拟低光环境下的图像噪点）。在相关细粒度识别研究中，综合数据增强通常带来 1-3pp 的准确率提升。

**损失函数改进**，对比损失在本研究的 batch_size=32 配置下工作稳定，但随着数据规模扩大到 GPU 可支持的批量（128-512），可以考虑引入批内难例挖掘（Batch Hard Mining）三元组损失。Hermans 等人（2017）在多个度量学习基准上证明，难例挖掘策略相比随机负采样能带来 2-5pp 的准确率提升，特别是在类别数量大（本研究 1393 类）、类内差异大的场景[8]。另一个值得探索的方向是 ArcFace 损失[19]，其角度间隔约束比对比损失能更严格地在嵌入空间中组织类别分布。
...
**鼻部区域精确定位**，当前方案直接对用户上传的全图进行 Resize 到 224×224 处理，若用户拍摄时鼻部只占图像的小部分，有效纹路区域的分辨率会大幅下降。引入一个轻量的鼻部检测器（如基于 YOLO 的单阶段检测器[14]），在孪生网络前先定位并裁剪鼻部区域，可以显著提升有效输入分辨率，有望带来 3-5pp 的准确率提升。另一种无需额外检测器的方案是引入空间变换网络（Spatial Transformer Network）[28]，让模型自主学习对输入进行几何对齐，以端到端方式解决拍摄角度偏差问题。
